...
The server validates the room code, UUID, strict boolean fields, year enum, and positive integer genre IDs; it deduplicates and sorts genre IDs before hashing a fixed-order allowlist of the filter values. Under a `rooms` row lock it verifies the room-scoped host credential and an unexpired editable state (`waiting` or `catalog_insufficient`), checks that genres exist, replaces the filters, and inserts a `room_filter_saves` receipt atomically. Saving from `catalog_insufficient` atomically returns the room to `waiting` and clears its exhaustion reason.
