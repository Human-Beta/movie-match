...
export default async function TvRoomPage({
  ...
  if (!room) {
    return <UnavailableRoom title={t("unavailableTitle")} description={t("unavailableDescription")} />;
  }
  ...
  return (
    <main className="min-h-screen bg-slate-950 px-6 py-10 text-slate-50 sm:py-16">
      <section className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-5xl items-center gap-12 lg:grid-cols-[1fr_22rem]">
        <div>
          <p className="mb-4 text-sm font-semibold tracking-[0.3em] text-amber-400 uppercase">Movie Match</p>
          <h1 className="text-4xl font-bold tracking-tight sm:text-6xl">{t("joinTitle")}</h1>
          <p className="mt-6 text-lg leading-8 text-slate-300">{t("joinDescription")}</p>
          <div className="mt-10">
            <p className="text-sm font-semibold tracking-[0.2em] text-slate-400 uppercase">{t("roomCodeLabel")}</p>
