...
test("projects a committed first ballot without waiting for a snapshot", () => {
  ...
  assert.deepEqual(reconcileBallotProgress({ projection, snapshot, snapshotEpoch: 1, snapshotReadFailed: false }), {
    submittedCount: 1,
    totalParticipants: 2,
...
test("replaces a local projection with the next authoritative snapshot", () => {
  ...
  assert.deepEqual(reconcileBallotProgress({ projection, snapshot: authoritativeSnapshot, snapshotEpoch: 2, snapshotReadFailed: false }), {
    submittedCount: 1,
    totalParticipants: 2,
    readyForResults: false,
  });
...
test("keeps an idempotent retry local only until snapshot failure or a fresh snapshot", () => {
  const snapshot = votingSnapshot();
  ...
  assert.deepEqual(
    reconcileBallotProgress({ projection: retryProjection, snapshot, snapshotEpoch: 1, snapshotReadFailed: true }),
    snapshot.ballotProgress,
  );
});
...
test("ignores a local projection after a realtime reconnect", () => {
  ...
  assert.deepEqual(reconcileBallotProgress({ projection, snapshot, snapshotEpoch: 2, snapshotReadFailed: false }), snapshot.ballotProgress);
});
